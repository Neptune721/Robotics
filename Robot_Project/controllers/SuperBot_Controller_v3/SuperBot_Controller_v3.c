...
{
  Init_Pose,      //初始状态
  Recognize_Empty,  //识别空货架状态
  Arround_Moving,  //四周环绕寻找货品状态
  Grab_Item,      //抓取物品状态
  Back_Moving,     //返程移动状态
  TurnBack_To_LoadItem, //转身装载货品状态
  Item_Loading,         //装载货品状态
  RunTo_NextShelf     //前往下一个货架状态
};
  ...
  case Recognize_Empty:
  {
    ...
    {      
      *main_state = Arround_Moving;
      printf("状态由识别空货架状态转移至环绕寻找所需物品状态!\n");
      set_posture(initial_posture, gps_values[0], gps_values[1], compass_angle);
...
bool Find_Empty(WbDeviceTag camera)
{
  ...
  for (int i = 0; i < 4;i++)
    for (int j = 0; j < 16;j++)
      GoodsonShelf[i][j] = -1;//先对所有进行初始化为-1
  for (int i = 0; i < number_of_objects; ++i)
  {
